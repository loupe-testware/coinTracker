...
function Authentication({ authState, setAuthState }: AuthenticationProps) {
  ...
  async function signUp() {
    try {
      ...
      setAuthToast({
        ...authToast,
        signUpSuccessToast: true,
        message: "Please check your email to get verification code",
      });
      setAuthState("confirmSignUp");
    } catch (err) {
      console.log({ err });
      setAuthToast({ ...authToast, errorToast: true, message: err.message });
    }
  }
      ...
      case "signIn":
        return (
          <div className="authContainer">
            ...
            />
            <button disabled={loadingSpinnerShow} onClick={signIn}>
              SIGN IN
            </button>
            {loadingSpinnerShow ? (
              <img
                className="signInLoadingSpinner"
                src={loadingSpinner}
                alt="loading spinner"
              />
            ) : null}
    ...
    <div className="backgroundFilter">
      <IonCard className="authCard">
        ...
        <IonToast
          ...
          onDidDismiss={() =>
            setAuthToast({ ...authToast, signUpSuccessToast: false })
          }
        ...
        <IonToast
          ...
          onDidDismiss={() =>
            setAuthToast({ ...authToast, authSuccessToast: false })
          }
          message={authToast.message}
          duration={3000}
          color="success"
